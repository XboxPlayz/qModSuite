...
public class ModModeHandler implements Listener
{
    ...
    private void giveStaffItem( Player player, Material mat, String name, ArrayList< String > lore, int slot )
    {
        ItemStack item = new ItemStack( mat, 1 );
        ItemMeta itemMeta = item.getItemMeta();
        itemMeta.setDisplayName( Color.msg( name ) );

        // FIXME: Check if getting lore from itemMeta and setting it directly from there causes issues
        for ( String s : Color.msg( lore ) )
        {
            itemMeta.getLore().add( s );
        }
    ...
    {
        // FIXME: If giving staff a colored carpet does not work: check this function signature
